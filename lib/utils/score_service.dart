...
class ScoreService {
  ...
  Future<List<LeaderboardEntry>> getLeaderboard() async {
    try {
      final res = await _db
          .from('scores')
          .select('username, score, max_score')
          .order('score', ascending: false);

      // Agregasi manual: group by username
      final Map<String, Map<String, int>> grouped = {};
      for (final row in res as List<dynamic>) {
        final name = row['username'] as String;
        grouped.putIfAbsent(name, () => {'total': 0, 'max': 0, 'games': 0});
        grouped[name]!['total'] = (grouped[name]!['total']! + (row['score'] as int));
        grouped[name]!['max']   = (grouped[name]!['max']!   + (row['max_score'] as int));
        grouped[name]!['games'] = (grouped[name]!['games']! + 1);
      }
